...
class FlagAssessor(dspy.Signature):
    """
    Analyze the text and determine if it should be recommended to the user based on the project context.
    
    CRITICAL INSTRUCTION: prioritize RECALL. If there is even a slight chance the content is relevant to the project, label it 'positive'.
    It is better to recommend irrelevant content (False Positive) than to miss relevant content (False Negative).
    """
...
def maximize_recall_metric(gold, pred, trace=None):
    ...
    gold_bool = str(gold.label).lower() == 'positive'
    pred_bool = str(pred.prediction).lower() == 'positive'
    
    # CASE 1: True Positive (The Goal)
    if gold_bool and pred_bool:
        return 1.0
        
    # CASE 2: False Negative (The Enemy) -> Strict punishment
    if gold_bool and not pred_bool:
        return 0.0
        
    # CASE 3: False Positive (Acceptable cost for high recall)
    # We give partial credit so the optimizer doesn't "unlearn" being aggressive
    if not gold_bool and pred_bool:
        return 0.7 
...
for pid in unique_projects:
    ...
    try:
        ...
        filename = f'maxed_flag_classifier_project_{str_pid}_{timestamp}.json'
